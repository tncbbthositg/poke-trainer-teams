...
test('loads the data explorer shell', async ({ page }) => {
  ...
  await expect(page.getByRole('checkbox', { name: 'Hide scarce candy' })).toBeChecked()
...
test('supports picking a battle team', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Battle Simulation', exact: true })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Battle Simulation Result' })).toBeVisible()
  ...
  await expect(page.getByText('Experimental proxy simulation')).toBeVisible()
  await expect(page.getByText(/Proxy (win|loss)/)).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Timeline' })).toBeVisible()
  await expect(page.getByText('Rocket').first()).toBeVisible()
  await expect(page.getByLabel(/Persian attacks/).first()).toBeVisible()
})
...
test('keeps pokemon explorer controls after visiting movesets', async ({ page }) => {
  await page.goto('/#/pokemon')
  ...
  await expect(page.getByRole('checkbox', { name: 'Hide scarce candy' })).not.toBeChecked()
})
