...
async function main() {
  const pokemon = pokemonSnapshotSchema.parse(await parse('public/data/pokemon.json'))
  const moves = movesSnapshotSchema.parse(await parse('public/data/moves.json'))
  const rocket = rocketLineupSnapshotSchema.parse(await parse('public/data/rocket-lineups.json'))
  ...
  for (const species of pokemon.candidates) {
    ...
    for (const move of species.chargedMoves) {
      if (!chargedIds.has(move)) {
        throw new Error(`${species.name} references unknown charged move ${move}`)
      }
    }
  }
  ...
  console.log(
    `Validated ${pokemon.candidates.length} candidates, ${moves.fastMoves.length} fast moves, ${moves.chargedMoves.length} charged moves, and ${rocket.lineups.length} Rocket lineups.`,
  )
}
